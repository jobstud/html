...
function bingoCheck(){
    ...
    $(this).css('background','black');  // this는 현재 함수를 실행한 객체
    ...
    */
    ...
    for(var i=0; i<5; i++){
        ...
        if(bingo[i*4+4]==0) 대각선2++;   // 4, 8, 12, 16, 20    (4로 출발하기 때문에 +4가 필요하다.)
        if(대각선1==5) end++;
        if(대각선2==5) end++;
        if(가로==5) end++;
        if(세로==5) end++;
...
function endGame(){
    $(".num").off();    // ()모든 이벤트 제거. 클래스가 num인 이벤트를 제거하기 위함. .off('click'); -> 특정 이벤트만 제거
    // 플레이 타임 멈추기
